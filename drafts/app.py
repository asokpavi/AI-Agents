...
# Set your OpenRouter API key (make sure it's correctly set in your environment variables or hardcoded for testing)
...
def generate_metrics_and_risks(product_name, user_problem, key_features, prd_text):
    ...
    response = requests.post("https://openrouter.ai/api/v1/chat/completions", headers=headers, json=data)
# Split the response into metrics and risks
    if response.status_code == 200:
        result = response.json()
        ...
        if "⚠️" in content:
            ...
            # If the delimiter is missing, treat the entire content as metrics and leave risks empty
...
"""
    try:
        # Cheaper LLM call via OpenRouter (Mistral)
        ...
        # Handle new API format (OpenAI client for OpenRouter returns objects, not dicts)
...
st.title("🧠 PRD Synthesizer - Multi-Agent Edition")

# PM Synthesizer Agent
def synthesize_feedback(prd_text, designer_feedback, engineer_feedback,metrics_output):
    prompt = f"""
    You are a Staff Product Manager. Given the PRD and feedback from Designer and Engineer, refine the PRD.
    ...
    Return an improved PRD in bullet format.
    """
    try:
        ...
        return f"PM Synthesizer Error: {str(e)}"
...
graph.add_edge("Generate PRD", "Generate Designer Feedback")
...
if st.button("Generate PRD"):
    ...
    result_state = runnable_graph.invoke(initial_state)
